...
if(isset($_POST['create'])){
	...
	if(isset($_POST['profpic']) || isset($_POST['id']) || isset($_POST['fname']) || isset($_POST['lname']) || isset($_POST['address']) || isset($_POST['bdate']) || isset($_POST['phone']) || isset($_POST['email']) ){
		...
		if (empty($picture)) {
			...
			$sql = " UPDATE users SET fname='$fname', lname='$lname', paddress='$address', birthdate='$bday', phonenumber='$phone', email='$email', profilepicture='$image'  WHERE id='$userid'";
			$result = mysqli_query($connection, $sql);
			...
			if($result){
				echo "<script> alert('Data Has Been Updated'); window.location='/Modules/dentist/index.php'  </script>";
			}
